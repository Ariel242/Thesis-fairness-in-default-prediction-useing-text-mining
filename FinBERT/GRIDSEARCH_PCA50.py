...
Structured + FinBERT (PCA-50)
=============================================================================================
Companion to this folder's BASELINE_PCA50.py and GRIDSEARCH_768.py — the
fourth and last of this folder's four-way comparison:

                    fixed hyperparams          grid search (per fold)
  768-dim (raw)     BASELINE_768.py            GRIDSEARCH_768.py
  50-dim (PCA)      BASELINE_PCA50.py          GRIDSEARCH_PCA50.py  <- this file

Feature matrix identical to BASELINE_PCA50.py (structured block + has_desc +
50 PCA components of sem_*). Grid search protocol identical to
GRIDSEARCH_768.py / analysis/GRIDSEARCH.py: within each fold, the last 20%
of that fold's (chronologically ordered) training rows are held out as a
temporal validation split; imputer/scaler/PCA for both blocks are fit on the
remaining 80% ("sub-train") only; every grid combination is scored on
validation AUC; the winner is refit on the FULL fold training set (fresh
imputer/scaler/PCA) before scoring the actual test fold. PCA itself is
therefore fit THREE times per fold: once on sub-train (for the search),
once on the full fold train (for the final fit) — same discipline as every
other fold-local transform in this project, just applied twice because of
the extra tuning step.

GRIDS: identical to GRIDSEARCH_768.py, including the one deviation from
analysis/GRIDSEARCH.py (Logistic's C grid extended to include 0.01).

OUTPUT (written under FinBERT/ only):
  results/gridsearch_pca50/wf_predictive_folds.csv (includes winning params)
  results/gridsearch_pca50/wf_best_params.csv
  results/gridsearch_pca50/wf_predictive_summary.csv
  results/gridsearch_pca50/wf_pca_variance.csv   — explained variance ratio per fold (final fit)
  figures/gridsearch_pca50/fig1_auc_stability.png
"""
...
GRID_LR = {"C": [0.01, 0.03, 0.1, 0.3, 1]}   # extended one step lower — see GRIDSEARCH_768.py docstring
...
# ============================================================
# 6. FEATURE MATRIX  (structured block + PCA-compressed FinBERT block, each fit on the given TRAIN rows only)
# ============================================================
def build_matrix(df_fit, df_apply):
    ...
    pca_fit   = pca.fit_transform(df_fit[SEM_COLS].values.astype(np.float32))
    pca_apply = pca.transform(df_apply[SEM_COLS].values.astype(np.float32))
    evr = float(pca.explained_variance_ratio_.sum())

    fb_fit_raw   = np.hstack([df_fit[["has_desc"]].values.astype(np.float32), pca_fit])
    fb_apply_raw = np.hstack([df_apply[["has_desc"]].values.astype(np.float32), pca_apply])
...
# 8. MAIN LOOP
# ============================================================
pred_rows, best_params_rows, pca_rows = [], [], []
...
for f in folds:
    ...
    if can_search:
        X_sub, X_val, _evr_search = build_matrix(df_sub, df_val)
...
print("\n" + "="*70)
print("A. PREDICTIVE PERFORMANCE SUMMARY (mean ± std across folds) — Structured + FinBERT-PCA50, Grid Search")
print("="*70)
...
print("\n" + "="*70)
print("C. PCA EXPLAINED VARIANCE PER FOLD (final fit, full fold train)")
print("="*70)
...
# 10. SAVE
# ============================================================
out_dir = SCRIPT_DIR / "results" / "gridsearch_pca50"
...
figures_dir = SCRIPT_DIR / "figures" / "gridsearch_pca50"
...
fig.suptitle("Walk-Forward AUC Stability — Structured + FinBERT-PCA50, Grid-Searched\n(shaded band = 95% bootstrap CI)",
