"""
Save raw per-fold test predictions — Structured + FinBERT-768 (raw), fixed hyperparameters
=============================================================================================
Lightweight companion to BASELINE_768.py, built to support DELONG_768_VS_PCA50.py after
that combined script twice hit the same OOM wall (fold 11/14) fitting both the 768-dim and
PCA-50 variants within one long-running process. This script does ONLY what's needed to
produce a DeLong-ready prediction file: same data load, same feature matrix, same fixed
hyperparameters as BASELINE_768.py, but no bootstrap CI, no figures, no feature importance
— and it exits after saving, so the OS fully reclaims memory rather than relying on
in-process gc between phases. Run this and SAVE_PREDICTIONS_PCA50.py as two SEPARATE
`python` invocations (never combined in one process) — that's the actual fix.

OUTPUT: results/predictions_768.csv — columns: fold, Model, id, y_true, p_pred
"""
...
out_path = out_dir / "predictions_768.csv"

# --- Checkpoint/resume: this process has been killed (OOM or otherwise)
# partway through in every prior attempt. Rather than redo already-completed
# folds every retry, skip any fold whose predictions are already on disk,
# and append (not overwrite) after each fold, so a kill mid-run only costs
# the folds since the last checkpoint, not the whole run. ---
done_folds = set()
if out_path.exists():
...
for f in folds:
    ...
    # Checkpoint immediately — this fold's work survives even if the next one gets killed.
