...
struct_cols = struct_cols + [c for c in num_feat_cols if c not in struct_cols]
print(f"  Total structured features (incl. text-numeric): {len(struct_cols)}")
...
for f in folds:
    ...
    # --- Structured features: impute then scale, fit on train only ---
    ...
    # Combined matrix
    X_tr_full = hstack([csr_matrix(X_tr_s), X_tr_txt])
    X_te_full = hstack([csr_matrix(X_te_s), X_te_txt])
